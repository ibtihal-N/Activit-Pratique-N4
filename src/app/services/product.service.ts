...
export class ProductService {
  ...
  public getProducts(page : number=1, size : number=4){
    return this.http.get(`http://localhost:8088/products?_page=${page}&_limit=${size}`,{observe:'response'});
  }
  ...
  public searchProducts(keyword:string):Observable<Array<Product>>{
    return this.http.get<Array<Product>>(`http://localhost:8088/products?name=${keyword}`);
  }
}
